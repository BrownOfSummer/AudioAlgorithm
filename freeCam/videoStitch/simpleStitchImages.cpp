...
float match_conf = 0.65f; // surf use 0.65 more robust than orb
...
int panorama_it(vector<string> &img_names, Mat &ret)
{
#if ENABLE_LOG
    ...
    if( indices.size() != img_names.size() ) 
    {
        printf("Not all images matched well !\n");
        return -1;
    }
...
void help()
{
    ...
    printf("Modify for stitchPlane, slow version.\n");
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 1; i < argc; i++) imgPaths.push_back(argv[i]);
    int num = static_cast<int>(imgPaths.size());

    // prepare paras
    Mat result, result_mask, cropped;
    Rect rect;
    int flag = panorama_it(imgPaths, result);
    if(flag == 0)
    {
        result.convertTo(result, CV_8U);
        bool crop_flag = cropimg2rect(result, rect);
        if( crop_flag )
        {
            cropped = result(rect);
            imwrite("result.jpg", cropped);
            printf("save to result.jpg");
        }
        else
        {
            printf("Warning, crop failed, save to result.jpg\n");
            imwrite("result.jpg", result);
        }
    }
    else
        printf("ERROR, Panorama faild.\n");
