...
int main(int argc, char *argv[])
{
    ...
    if( !isVideo )
    {
        ...
        {
            string imagePath = string( imageDir ) + imageNames[i]; // imageDir like ../path/
            ...
            else printf("No found in %s\n", imageNames[i].c_str());
        }

        /*save abspath to selected.txt*/
        if( selectedFilePath.size() > 0 )
        {
            printf("Save the path to selected.txt\n");
            FILE *fp = fopen("selected.txt","w");
            if( fp == NULL ) {
